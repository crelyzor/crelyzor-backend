import { escapeHtml } from "../../../utils/security/htmlEscape";

interface TeamInviteTemplateArgs {
  inviterName: string;
  teamName: string;
  role: "ADMIN" | "MEMBER";
  acceptUrl: string;
...
// Mirrors the visual language of adminInvite.ts. Every interpolation from a
// user-controlled source (inviterName, teamName, message) is escaped via
// escapeHtml — the URL is constructed server-side from a hex-only token so
// it's safe to embed without escape.
          ...
          <tr>
            <td>
              ...
              <h1 style="margin:0 0 16px;font-size:22px;font-weight:600;color:#fafafa;line-height:1.2;">
                You've been invited to ${safeTeam}
              </h1>
              <p style="margin:0 0 24px;font-size:14px;color:#a3a3a3;line-height:1.6;">
                ${safeInviter} invited you to join <strong style="color:#fafafa;">${safeTeam}</strong> on Crelyzor as a <strong style="color:#fafafa;">${roleLabel}</strong>.
              </p>
              ${
                safeMessage
                  ? `<div style="margin:0 0 24px;padding:16px;background:#0a0a0a;border:1px solid #262626;border-radius:8px;font-size:13px;color:#d4d4d4;line-height:1.6;white-space:pre-wrap;">${safeMessage}</div>`
                  : ""
              }
